...
public class CustomSlidingUpPanelLayout extends ViewGroup {

    private static final String TAG = CustomSlidingUpPanelLayout.class.getSimpleName();
    ...
    public CustomSlidingUpPanelLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CustomSlidingUpPanelLayout);

            if (ta != null) {
                mPanelHeight = ta.getDimensionPixelSize(R.styleable.CustomSlidingUpPanelLayout_panelHeight, -1);
                mShadowHeight = ta.getDimensionPixelSize(R.styleable.CustomSlidingUpPanelLayout_shadowHeight, -1);
                mParalaxOffset = ta.getDimensionPixelSize(R.styleable.CustomSlidingUpPanelLayout_paralaxOffset, -1);

                mMinFlingVelocity = ta.getInt(R.styleable.CustomSlidingUpPanelLayout_flingVelocity, DEFAULT_MIN_FLING_VELOCITY);
                mCoveredFadeColor = ta.getColor(R.styleable.CustomSlidingUpPanelLayout_fadeColor, DEFAULT_FADE_COLOR);

                mDragViewResId = ta.getResourceId(R.styleable.CustomSlidingUpPanelLayout_dragView, -1);

                mOverlayContent = ta.getBoolean(R.styleable.CustomSlidingUpPanelLayout_overlay,DEFAULT_OVERLAY_FLAG);
            }
